...
searchbtn.addEventListener("click", () => {
  ...
  filteredProducts.forEach((product) => {
    mobileCards1.innerHTML = `<div href="./products.html" class="col-sm-3 col-12 fw-bold p-2 border border-secondary-subtle rounded-2"">
    <a href="./products.html?id=${product.id}" class="image">
...
rangeBtn.addEventListener("click", () => {
  ...
  filteredPrices.forEach((price) => {
    ...
    <a href="./products.html?id=${price.id}" class="image">
...
const displayCarts = () => {
  fetch(`https://dummyjson.com/products/search?q=phone&limit=8&id=${products.id}`)
  .then(res => res.json())
  .then(data => {
    console.log(data.products)
    mobileCards1.innerHTML = ""
    data.products.forEach((product) => {
      mobileCards1.innerHTML += `<div  class="product-container col-md col-sm-12 fw-bold p-2 border border-secondary-subtle rounded-2"  style="min-width: 20.5svw;">
      <a href="./products.html?id=${product.id}" class="image">
      <img src="${product.images[0]}" alt="" class="w-100 rounded-2" style="height: 190px">
      </a>
      <div class="products-container">
        ...
        <h3 class="text-dark fs-4 pt-2 products">${product.title}</h3>
        <p class="description text-secondary">${product.description}</p>
        <p class="green-span text-success">${Math.round((product.price) * 100) / 100}$ <small class="red-span text-danger" style>${Math.round((product.price * (100 - product.discountPercentage) / 100) * 100) / 100}$</small></p>
